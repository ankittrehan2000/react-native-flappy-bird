...
const updateObstacle = (entities, {time, dispatch}) => {
  ...
  for(let i = 1; i <=2; i++){
    if(
      entities['Obstacle' + i].type === 'top' && entities['Obstacle' + i].body.position.x <= -1 * (Constants.TOP_PIPE_WIDTH / 2)
    ){
      ...
      entities['Obstacle' + i].type === 'bottom' & entities['Obstacle' + i].body.position.x <= -1 * (Constants.BOTTOM_PIPE_WIDTH / 2)
    ){
      entities['Obstacle' + i].scored = false;
      Matter.Body.setPosition(entities['Obstacle' + i].body, {
